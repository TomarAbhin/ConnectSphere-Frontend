...
export default function RegisterPage() {
  ...
  return (
    <div className="auth-surface mx-auto grid min-h-screen max-w-6xl items-center gap-8 px-4 py-8 lg:grid-cols-[0.95fr_1.05fr]">
      {/* Left hero */}
      ...
      <div className="glass-card-static p-8 animate-fade-in-up">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4" autoComplete="off">
          <div className="grid gap-4 sm:grid-cols-2">
            <input name="username" autoComplete="off" required value={form.username} onChange={handleChange} placeholder="Username" className="cs-input" />
            <input name="fullName" autoComplete="off" value={form.fullName} onChange={handleChange} placeholder="Full name" className="cs-input" />
          </div>
          <input name="email" autoComplete="off" type="email" required value={form.email} onChange={handleChange} placeholder="Email" className="cs-input" />
          <input name="password" autoComplete="new-password" type="password" required value={form.password} onChange={handleChange} placeholder="Password (min 8 characters)" className="cs-input" />
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="flex h-[calc(100%-1.375rem)] items-center rounded-2xl border border-dashed border-white/10 bg-white/5 px-4 text-xs text-slate-400">
          ...
          {form.role === 'ADMIN' ? (
            <div className="animate-slide-down">
              <input name="adminSecretKey" autoComplete="new-password" value={form.adminSecretKey} onChange={handleChange} placeholder="Admin invite key" className="cs-input" />
        ...
        <p className="mt-6 text-center text-sm text-slate-400">
          ...
          <Link to="/login" className="font-semibold text-brand-300 hover:text-white transition-colors">Login</Link>
        </p>
      </div>
    </div>
  );
